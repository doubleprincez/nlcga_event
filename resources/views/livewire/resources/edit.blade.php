...
new #[Layout('components.layouts.app')] class extends Component
{
    ...
    public function mount(Resource $resource): void
    {
        ...
        $this->fill($resource->only(['title', 'description', 'file_url', 'preview_link', 'post_type', 'status', 'conference_year']));
    }

    public function save(): void
    {
        $this->validate(['title' => 'required|string|max:255']);
        $this->resource->update($this->only(['title', 'description', 'file_url', 'preview_link', 'post_type', 'status', 'conference_year']));
...
<div>
    <div class="flex items-center gap-3 mb-6">
        ...
        <flux:heading size="xl">Edit Resource</flux:heading>
    </div>
    <form wire:submit="save" class="max-w-2xl space-y-4">
        <flux:card>
            <div class="space-y-4">
                ...
                <flux:field><flux:label>File URL</flux:label><flux:input wire:model="file_url" /></flux:field>
                <flux:field><flux:label>Preview Link</flux:label><flux:input wire:model="preview_link" /></flux:field>
